...
@Composable
fun HistoryUserView(navController: NavController, viewModel: HistoryUserViewModel) {
...
@Composable
private fun MyAccessItem(
    number: Int,
    UidMyAccess: String,
    DataAccess: AccessUserModel,
    navController: NavController,
    viewModel: HistoryUserViewModel
) {
    val tipoDeRegistro =
    ...
    Column(
        modifier = Modifier
            .clickable() {
                navController.navigate("my_access_detail/$UidMyAccess")
            }
    ) {
        ListItem(
            modifier = Modifier
